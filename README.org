...
- [[#pause][pause (Beamer overlay)]] macro
- [[#style][style]] macro
...
** Fnbeamer (Beamer footnote) macro
:PROPERTIES:
:CUSTOM_ID: fnbeamer
:END:

The ~fnbeamer~ macro inserts a raw LaTeX footnote using the ~\footnote{...}~
command.
...
#+begin_src org
,#+MACRO: fnbeamer @@latex:\footnote{$1}@@
#+end_src
...
#+begin_src org :tangle no
Here is some text {{{fnbeamer(This is a footnote.)}}}.
#+end_src
...
** pause (Beamer overlay) macro
:PROPERTIES:
:CUSTOM_ID: pause
:END:

The ~pause~ macro provides a shortcut for inserting an inline Beamer pause in
slides exported with ox-beamer.

It wraps the Beamer ~\pause~ command using Org's backend-specific inline syntax.

The macro is intended to simplify the writing of slides where sentences,
paragraphs, examples, or other content should appear progressively, without
using lists, blocks, or dedicated Beamer overlay specifications.
...
It performs a direct textual substitution into Beamer inline code at export
time.
...
When exporting to Beamer, each occurrence of ~{{{pause}}}~ is replaced with
~\pause~, causing the sentences to appear on successive overlays.

*** Important

The Beamer ~\pause~ command should not be used together with frames configured
with:
...
~\pause~, only the first frame content may be displayed or exported correctly.
...
- Use ~{{{pause}}}~, overlay specifications such as ~<+->~, or explicit Beamer
  overlay commands when progressive disclosure is required.
