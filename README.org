...
*Remark --* When macros are disguised calls to Babel code, they must be on their
own on a line: /nothing/ before nor after (/but spaces/).
...
The =author= macro display a list of users who have created or edited the
document.
...
** Date macro (current time) XXX
   :PROPERTIES:
   :CUSTOM_ID: date
   :END:

The =date= macro outputs the *current date and time* /when/ the document is being
exported.
...
This document was last exported on: {{{date(%Y-%m-%d %a %H:%M)}}}. XXX
...
The =email= macro displays the email of the author(s).
...
The =title= macro displays the document title.
